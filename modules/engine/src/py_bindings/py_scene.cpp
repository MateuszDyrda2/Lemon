...
PYBIND11_EMBEDDED_MODULE(scene, m)
{
    ...
    py::class_<scriptable_entity>(m, "scriptable_entity")
        ...
        .def("translate", [](scriptable_entity& ent, const vec3& by) { ent.ent.patch_component<transform>([&by](transform& t) { t.position += by; }); })
        .def("rotate", [](scriptable_entity& ent, f32 by) { ent.ent.patch_component<transform>([&by](transform& t) { t.rotation += by; }); })
        .def("scale", [](scriptable_entity& ent, const vec3& by) { ent.ent.patch_component<transform>([&by](transform& t) { t.scale += by; }); })
