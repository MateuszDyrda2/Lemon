    ...
    const mat4& trans, color col, vec4 texCoords)
{
    ...
    vec4 colData = { col.r, col.g, col.b, col.a };
...
void batch_renderer::render_sprite(const color& col, const vec4& texCoords, asset<texture>& tex,
                                   const mat4& model)
{
    ...
    found->add_quad(model, col, texCoords);
}
